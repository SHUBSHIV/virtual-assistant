    ...
    return (
        <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
            <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handleSignIn}>
                <h1 className='text-white text-[30px] font-semibold'>Sign in to
                ...
                <div className='w-full h-[60px] border-2 border-white bg-transparent text-white rounded-full text-[18px] relative'>
                    <input
                        ...
                        value={password} 
                    />
                    <div onClick={() => setShowPassword(!showPassword)} className='absolute top-[18px] right-[20px] cursor-pointer'>
                        {showPassword ? <IoEyeOff className='w-[25px] h-[25px] text-white' /> : <IoEye className='w-[25px] h-[25px] text-white' />}
                    </div>
                </div>
